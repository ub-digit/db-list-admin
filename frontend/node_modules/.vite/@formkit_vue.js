...
function createMessages(node, ...errors3) {
  ...
  return errors3.filter((m) => !!m).map((errorSet) => {
    ...
    if (Array.isArray(errorSet)) {
      ...
      for (const key in errorSet) {
        if (Array.isArray(errorSet[key])) {
          errors4[key] = errorSet[key].map((error2) => make(error2));
        } else {
          errors4[key] = [make(errorSet[key])];
        }
      }
      return errors4;
    }
...
function errors(node, _context, localErrors, childErrors) {
  const sourceKey = `${node.name}-set`;
  createMessages(node, localErrors, childErrors).forEach((errors3) => {
...
var validation$5 = {
  ...
  date_between({ name, args }) {
    return `${sentence(name)} musi by\u0107 pomi\u0119dzy ${date3(args[0])} i ${date3(args[1])}`;
  },
  email: "Prosz\u0119 wpisa\u0107 poprawny adres email.",
  ends_with({ name, args }) {
    return `${sentence(name)} nie ko\u0144czy si\u0119 na ${list2(args)}.`;
  },
  is({ name }) {
  ...
  starts_with({ name, args }) {
    return `${sentence(name)} nie zaczyna si\u0119 od ${list2(args)}.`;
  },
  url() {
...
var defaultConfig2 = (options = {}) => {
  const _a = options, { rules = {}, locales = {}, inputs: inputs$1 = {}, messages: messages2 = {}, locale = void 0 } = _a, nodeOptions = __objRest(_a, ["rules", "locales", "inputs", "messages", "locale"]);
